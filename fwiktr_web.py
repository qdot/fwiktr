...
class Fwiktr:
    ...
    def RunPOSTagger(self):
        ...
        for message in twitter_messages:
            try:
                self.fwiktr_info = {'twitter_author':'', 'twitter_location':'', 'chain_mechanism':1, 'selection_mechanism':1, 'post_source':1, 'post_url':'', 'post_date':'', 'post_text':message.text}
                ...
                self._AddFwiktrData(dict([('post_pos_output', "\n".join(["\t".join(i) for i in tokens]))]))
    ...
    def _RetreiveFlickrURLs(self, tagList):
        ...
        self._AddFwiktrData(dict([('flickr_total', rsp.photos[0]['total']), ('post_tags',tag_string)]))
        
	a = rsp.photos[0].photo[rand_index]
        self._AddFwiktrData(dict([('flickr_server', a['server']), ('flickr_farm', a['farm']), ('flickr_photoid', a['id']), ('flickr_secret', a['secret']), ('flickr_ownerid', a['owner']), ('flickr_title', a['title'])]))
